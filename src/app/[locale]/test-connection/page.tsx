...
export default function TestConnectionPage() {
  ...
  const testTable = async (tableName: string) => {
    ...
    try {
      ...
      const data = await response.json();
      alert(`Table ${tableName}: ${data.status}${data.error ? ` - ${data.error}` : ''}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className={`mb-6 p-4 rounded-lg ${
              status?.status === 'healthy' ? 'bg-green-50 border border-green-200' :
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h4 className={`font-medium ${
                    status?.status === 'healthy' ? 'text-green-800' :
            ...
            {status?.connectivity && (
              <div className="mb-6">
                <h4 className="font-medium text-gray-900 mb-3">Table Connectivity</h4>
                <div className="mb-2">
                  <span className="text-sm text-gray-600">Overall: {status.connectivity.overall}</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(status.connectivity.details).map(([table, result]) => (
                    <div key={table} className="flex items-center justify-between p-3 border rounded">
                      ...
                      >
                        {testingTable === table ? 'Testing...' : result === 'success' ? '✓ Success' : '✗ Failed'}
                      </button>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-sm text-gray-500">
                Last tested: {status?.timestamp ? new Date(status.timestamp).toLocaleString() : 'Never'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="font-medium text-blue-900 mb-2">Connection Setup Instructions</h4>
          <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
            ...
            <li>Fill in your Supabase project credentials from the Supabase Dashboard</li>
            <li>Ensure you have run the database migration in Supabase SQL Editor</li>
