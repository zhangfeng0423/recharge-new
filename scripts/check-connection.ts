  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        error(`${table}: Not accessible - ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
    }

    // Check if all tables are accessible
    const accessibleTables = Object.values(result.database.tables).filter(Boolean).length;
...
function loadEnvFile() {
  ...
  try {
    ...
    info('Make sure to copy .env.local.example to .env.local and fill in your credentials');
  }
}
...
function generateNextSteps(result: ConnectionTestResult): string[] {
  ...
  if (!result.database.accessible) {
    ...
    steps.push('Copy content from supabase/migrations/20250109_001_initial_schema.sql');
  ...
  if (inaccessibleTables.length > 0) {
    steps.push(`Verify ${inaccessibleTables.join(', ')} tables exist in your database`);
  }
...
async function main() {
  ...
  switch (result.overall) {
    case 'success':
      success('All checks passed! Your database connection is working properly.');
      break;
    case 'warning':
      warning('Some issues detected. Database is accessible but not fully configured.');
      break;
    case 'error':
  ...
  if (nextSteps.length > 0) {
    ...
    log('\n✨ You can now start developing your Game Recharge Platform!', 'green');
  }
